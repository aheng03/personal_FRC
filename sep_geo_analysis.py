...
#######################################################################################################
#######################################################################################################
def comparison_plots(rz_og, rz_new, e, a, N, eps, Xs, title_txt):
    fig, ax = plt.subplots(figsize = (4, 6))
    ax.plot(rz_og[0], rz_og[1], label = r"$\boldsymbol{\psi} \boldsymbol{= 0}$", linestyle = "--", linewidth = 3, color='green')
    
    # ax.plot(rz_new[0], rz_new[1], label = r"$\boldsymbol{Z}_{\boldsymbol{S}}$")

    # zcap
    zcap = z_cap(rz_new[0], N, a, e, eps, Xs)
    # ax.plot(rz_new[0], zcap, label = r"$\boldsymbol{Z}_{\boldsymbol{cap}}$")

    ax.set_xlabel("r/R0", weight = 'bold')
    ax.set_ylabel("z/R0", weight = 'bold')
    ax.set_aspect('equal')
    ax.set_xlim((-1.5*max(rz_new[0]), 1.5*max(rz_new[0])))
    ax.set_ylim((-1.3*max(rz_new[1])), 1.3*max(rz_new[1]))
    # ax.legend(loc='center left', bbox_to_anchor=(1.1, 0.5),
                # frameon=False, borderaxespad=0.0)
    for label in ax.get_xticklabels() + ax.get_yticklabels():
        label.set_fontweight('bold')
    fig.savefig(f'output/{title_txt}')
...
for i, temp in enumerate(T):
    ylist_gamma_tear.append(gamma_tear(ylist_tau_clas[i], n_max_arr[i], B0, temp, Xs_arr, Rw, 1))


# Instabilities
ylist_tau_tilt = tau_tilt(np.array(ylist_gamma_tilt))
ylist_tau_tear = tau_tear(np.array(ylist_gamma_tear))

# Normalized
tau_clas_norm_ti = ylist_tau_clas /  (100*ylist_tau_tilt)
tau_LSX_norm_ti = ylist_tau_LSX / (100*ylist_tau_tilt)
tau_brem_norm_ti = ylist_tau_brems / (ylist_tau_tilt * 1e1)

tau_clas_norm_te = ylist_tau_clas / (ylist_tau_tear)
tau_LSX_norm_te = ylist_tau_LSX / (ylist_tau_tear)
tau_brem_norm_te = ylist_tau_brems / ylist_tau_tear

## === Shape Index
# N_arr               = np.zeros_like(elongation_arr)   # shape index
# for e_i, elong in enumerate(elongation_arr):
#     rz_og_arr.append(gs_(elong, 1, 1, Xs_arr[e_i]))
#     rz_arr.append(find_rz(rz_og_arr[e_i][0], rz_og_arr[e_i][1], a_arr[e_i], Xs_arr[e_i], 1))
#     eq, zs_func = poly_fit_eq(rz_arr[e_i][0], rz_arr[e_i][1], deg)
#     N_result = minimize_scalar(least_squares_fit_eps, args=(zs_func, a_arr[e_i], elong, 1, Xs_arr[e_i]), bounds=(1, elong), method='bounded')    
#     for i in range(len(T)):
#         N_arr[e_i] = N_result.x
#     comparison_plots(rz_og_arr[e_i], rz_arr[e_i], elong, a_arr[e_i], N_arr[e_i], 1, Xs_arr[e_i], f"Comparison Plot {e_i}")
#     print(f"index = {e_i}, E = {elong}, Xs = {Xs_arr[e_i]}, a = {a_arr[e_i]}, b = {b_arr[e_i]}")
...
# Flux Lifetimes versus Elongations [tilt]
for t_i in range(numTemps):
    fig2, ax2 = plt.subplots(figsize = (figXsize, figYsize))

    ax2.plot(elongation_arr, tau_clas_norm_ti[t_i], label = r'$\boldsymbol{\tau}_{\boldsymbol{classical}}  \ / \ 100\boldsymbol{\tau}_{\boldsymbol{tilt}}$', linestyle = lt_line_dict['clas'], color = T[t_i], linewidth = lineWidth)
    ax2.plot(elongation_arr, tau_LSX_norm_ti[t_i], label = r'$\boldsymbol{\tau}_{\boldsymbol{LSX}}  \ / \ 100\boldsymbol{\tau}_{\boldsymbol{tilt}}$', linestyle = lt_line_dict['LSX'], color = T[t_i], linewidth = lineWidth)
    # ax2.plot(elongation_arr, tau_brem_norm_ti[t_i], label = r'$\boldsymbol{\tau}_{\boldsymbol{brem}}   \ / \ \boldsymbol{10\tau}_{\boldsymbol{MHD}}$', linestyle = lt_line_dict['brem'], color = temp_color_dict[T[t_i]], linewidth = lineWidth)

    ax2.plot(elongation_arr, tau_clas_norm_te[t_i], label = r'$\boldsymbol{\tau}_{\boldsymbol{classical}}  \ / \ \boldsymbol{\tau}_{\boldsymbol{tear}}$', linestyle = lt_line_dict['clas'], color = T[t_i], linewidth = lineWidth)
    ax2.plot(elongation_arr, tau_LSX_norm_te[t_i], label = r'$\boldsymbol{\tau}_{\boldsymbol{LSX}}  \ / \ \boldsymbol{\tau}_{\boldsymbol{tear}}$', linestyle = lt_line_dict['LSX'], color = T[t_i], linewidth = lineWidth)
    ...
    fig2.tight_layout()
    fig2.savefig(f"{T[t_i]} Flux Lifetime [tilt] vs Elongation")


# Flux Lifetimes versus Elongations [tearing]
...
for t_i in range(numTemps):
    ...
    ax3.plot(elongation_arr, tau_clas_norm_te[t_i], label = r'$\boldsymbol{\tau}_{\boldsymbol{classical}}  \ / \ \boldsymbol{\tau}_{\boldsymbol{tear}}$', linestyle = lt_line_dict['clas'], color = T[t_i], linewidth = lineWidth)
    ax3.plot(elongation_arr, tau_LSX_norm_te[t_i], label = r'$\boldsymbol{\tau}_{\boldsymbol{LSX}}  \ / \ \boldsymbol{\tau}_{\boldsymbol{tear}}$', linestyle = lt_line_dict['LSX'], color = T[t_i], linewidth = lineWidth)

    ax3.legend(framealpha = 1, frameon = True, fontsize = legendFontSize)
    ax3.grid(alpha = 0.5)
    ...
    ax3.set_ylabel("Flux Lifetime", weight = 'bold', fontsize = textFontSize)

    fig3.tight_layout()
    fig3.savefig(f"{T[t_i]} Flux Lifetime [tear] vs Elongation")
